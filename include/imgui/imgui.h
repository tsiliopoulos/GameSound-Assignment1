...
namespace ImGui
{
    ...
    IMGUI_API void          SetWindowFontScale(float scale);                                    // per-window font localScale. Adjust IO.FontGlobalScale if you want to localScale all windows
    ...
    IMGUI_API float         GetFontSize();                                                      // get current font size (= height in pixels) of current font with current localScale applied
...
struct ImGuiIO
{
    ...
    float         FontGlobalScale;          // = 1.0f               // Global localScale all fonts
...
struct ImDrawData
{
    ...
    IMGUI_API void ScaleClipRects(const ImVec2& sc);  // Helper to localScale the ClipRect field of each ImDrawCmd. Use if your final output buffer is at a different localScale than ImGui expects, or if there is a difference between your window resolution and framebuffer resolution.
};
...
struct ImFont
{
    ...
    float                       Scale;              // = 1.f        // Base font localScale, multiplied by the per-window font localScale which you can adjust with SetFontScale()
