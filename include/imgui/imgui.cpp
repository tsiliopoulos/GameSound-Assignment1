...
!- color: the color helpers/typing is a mess and needs sorting out.
 ...
 - plot: "smooth" automatic localScale over time, user give an input 0.0(full user localScale) 1.0(full derived from value)
 ...
 - style: global localScale setting.
...
bool ImGui::SliderBehavior(const ImRect& frame_bb, ImGuiID id, float* v, float v_min, float v_max, float power, int decimal_precision, ImGuiSliderFlags flags)
{
    ...
    if (g.ActiveId == id)
    {
        if (g.IO.MouseDown[0])
        {
            ...
            if (is_non_linear)
            {
                // Account for logarithmic localScale on both sides of the zero
